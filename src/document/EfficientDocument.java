...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		// Not needed by Rick ArrayList <String> lastElementCheck = new ArrayList<String>(tokens);
		// TODO: Finish this method.  Remember the countSyllables method from 
		// Document.  That will come in handy here.  isWord defined above will also help.
		for (String t: tokens) {
			if (isWord(t)) {
				numWords++;
				numSyllables = numSyllables + countSyllables(t);
			}
			else numSentences++;
			
		}
		/*if(isWord(lastElementCheck.get(lastElementCheck.size()-1))) {
			numSentences++;
		}*/
		if (tokens.size()==0) {
		...
		else if(isWord(tokens.get(tokens.size()-1))){
			numSentences++;
		}
